...
describe("Genre Unit Tests", () => {
  ...
  test('create method', () => {
    ...
    expect(Array.from(genre.categories_id.keys())).toStrictEqual([categoryId.value]);
    expect(genre.categories_id.get(categoryId.value)).toBeInstanceOf(CategoryId);
  ...
  test("getter and setter of name prop", () => {
    const genere = Genre.fake().aGenre().build();
    expect(genere.name).toBe(genere.props.name);

    genere["name"] = "other name";
    expect(genere.name).toBe("other name");
  });
