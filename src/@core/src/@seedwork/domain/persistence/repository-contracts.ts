...
> {
  ...
  existsById(ids: EntityId[]): Promise<[EntityId[], EntityId[]]>;
...
type SearchResultProps<E extends AggregateRoot, Filter> = {
  items: E[];
  total: number;
  current_page: number;
  per_page: number;
...
export class SearchResult<E extends AggregateRoot = AggregateRoot, Filter = string> {
  readonly items: E[];
